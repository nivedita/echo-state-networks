...
# Step 4 - Detect outliers - MAD approach
k = 15
median = np.median(data)
mad = np.median(np.abs(data-median))
deviation = abs(data - median)
data[deviation > (k * mad)] = 1
data[deviation <= (k * mad)] = 0
...
fileName = "/Outlier_Detection_Using_MAD.html"
actualDataPlot = plot.OutputTimeSeriesPlot(outputFolderName+fileName, "Facebook Fans Change", "Outlier Detection using MAD approach", "Fans Change")
