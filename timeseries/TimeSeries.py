...
class TimeSeriesProcessor:
    ...
    def __init__(self, inputData, depth, horizon, valueIndex):
    ...
    def __generate__(self):
        ...
        while index <= (self.inputData.shape[0] - self.horizon):
            ...
            for depth in depthValues:
                processedData[dataCount, colCount] = self.inputData[index - depth, self.valueIndex]
                colCount += 1

            # Future values
            horizonValues = range(0, self.horizon, 1)
            for horizon in horizonValues:
                processedData[dataCount, colCount] = self.inputData[index + horizon, self.valueIndex]
                colCount += 1
